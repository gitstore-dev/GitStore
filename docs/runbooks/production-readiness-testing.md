...
Two concrete shapes exist in this repo, both correct, pick based on what
you're testing:
...
- **Sequential dual-instance (idempotency across restart), no goroutines
  ...
  (the bounded page isn't drained yet). A **second, independently
  constructed** reconciler instance (`secondController`, same shared fake) is
  then reconciled and asserted to reach `types.Success`, with the fake's
  counters proving the drain-then-complete sequence ran exactly once
  end-to-end (`decouples == 2`, `completions == 1`) despite two separate
  reconciler instances touching the same terminating record. A final
  reconcile after the record is deleted from the cache asserts a
  `types.TerminalFailure` and that `completions` is still `1` — a stale
  requeue after completion must not run `CompleteDeletion` a second time.
  There are no `go func` goroutines in this test; "two replicas" is modeled
  as two sequential reconciler instances sharing one mutex-guarded fake, which
  is sufficient to prove idempotency and is cheaper to write and debug than
  real concurrency.
- **Concurrent goroutines racing a real Runner+Manager pair across a
  simulated restart.** Worked example:
  `gitstore-controller-manager/tests/integration/reconcile_retry_resume_test.go`,
  `TestIntegration_Restart_ResumesFromCheckpoint_NoLostOrDuplicateWork`. Run 1
  starts a real `listwatch.Runner` and `manager.Manager` each in their own
  goroutine (`go func() { runnerDone1 <- runner1.Run(ctx1) }()` /
  `go func() { mgrDone1 <- mgr1.Start(ctx1) }()`), reconciles one item to
  success (tracked through a `sync.Mutex`-guarded `map[types.WorkItemKey]int`
  call counter), then is torn down (`cancel1(); <-runnerDone1; <-mgrDone1`)
  with a second item deliberately left in-flight. Run 2 constructs a fresh
  Runner+Manager against the same `checkpoint.Store` and asserts the
  already-completed item is not re-dispatched while the pending item resumes
  and completes — "no lost or duplicate work" across a restart, with the
  restore driven through the real checkpoint-persistence path rather than a
  hand-rolled fake.

Use the sequential shape when the reconciler-under-test is simple enough that
constructing two instances and calling `Reconcile` directly is enough to
prove the invariant. Reach for the goroutine + real Runner/Manager shape only
when the checkpoint/dispatch machinery itself is part of what you're
validating (e.g. a new `ListWatcher` implementation).
...
2. Pick the sequential or goroutine shape above based on whether you're also
   validating checkpoint/dispatch machinery.
...
Two clean, already-established worked examples exist for this — the concern
from the task brief that this pattern might have no real example turned out
to be unfounded:

- **Resolver-level (unit), fake `AuthZProvider`:**
  ...
  mutation and read path (`create`, `rename`, `transfer`, `delete`, `read by
  ID`, `read by namespace path`, `read through node`, `list namespace
  repositories`) is run as principal `bob` against resources owned by
  `alice`, asserting every single one returns exactly
  `"input: permission denied: cross-tenant access denied"` and that the
  authorizer was called with the correct `action` string, `resource.Kind`,
  `resource.OwnerSub`, and (for transfer) both source and target namespace
  attrs — proving the authorization check runs, and runs with correct
  context, on every mutation/read entry point, not just some of them. The
  companion `TestRepositoryResolverUsesOwnActionForTenantOwner` asserts the
  *positive* case: the same principal reading their own resource is
  authorized under a distinctly-named `*.own` action rather than `*.any`,
...
- **Full-stack integration (real GraphQL, two real users):**
  ...
  actor — proving isolation holds through the real resolver + datastore
  stack, not just against a fake.
...
fields is covered. It is not, because there is currently no `SecretRef`-style
field anywhere in this codebase (`grep -rn "SecretRef" gitstore-api` returns
zero matches as of this writing). This is genuinely not yet needed, not a
gap being glossed over — record it here so the first spec that *does*
introduce a secret-reference field knows to add a namespace-isolation
assertion for it (extend `assertRepositoryNamespaceIsolation`-style checks,
or the resolver-level table, to assert the field is never populated/visible
across the two users) rather than assuming an existing test already covers
it.
...
3. Add (or extend) a full-stack integration test with two real users each
   ...
   is denied with the expected error text, and (b) a same-user list/read
   query never returns the other user's resource by name or attribute.
4. If your resource introduces any field that references a secret,
   credential, or other sensitive external identifier, add an explicit
   assertion that the field is absent/empty when read cross-namespace — do
   not assume existing isolation tests cover a field they were never written
   against.
