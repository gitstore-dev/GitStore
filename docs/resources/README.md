...
| Binary variants                 | Object storage                         | `File/variants`, `MediaAsset/renditions`, `Invoice/pdf`                                                                | Store manifest and resolved URLs separately from payload bytes. `File` owns the technical asset lifecycle; `MediaAsset` is the catalog-facing semantic layer that points at a `File`. |
