...
func TestGraphQLHandlerRequiresAuthZProvider(t *testing.T) {
	...
	handler, err := app.NewGraphQLHandler(nil, nil, zap.NewNop(), registry, nil, nil, nil)
	require.Error(t, err)
...
func TestGraphQLHandlerAcceptsBearerTokenForNamespaceMutation(t *testing.T) {
	...
	handler, err := app.NewGraphQLHandler(store, &mockGitWriter{}, zap.NewNop(), newTestGraphQLRegistry(t), nil, apiruntime.NewSequenceIDGenerator(), nil)
	require.NoError(t, err)
