...
- Q: Should Namespace watch continue to be served only through the generic `watchResources(kind: String)` subscription, or should it get a dedicated, typed entry point? → A: Dedicated. Core, already-schema-stable resource kinds — starting with Namespace — MUST be observable through their own strongly-typed GraphQL subscription (e.g. `namespaceEvents`) rather than requiring every consumer to go through a stringly-typed `kind` parameter and a generic/union event payload that needs client-side type discrimination. This is a GraphQL schema-design correction, not a new transport or event-bus mechanism: the dedicated subscription is a thin, typed resolver in front of the same existing event bus and resumable-cursor machinery. The generic `watchResources(kind: "Namespace")` path is not removed by this spec (avoiding a breaking change for any existing caller), but the dedicated subscription becomes the canonical, documented entry point for Namespace going forward. Extending the same treatment to other already-shipped core kinds (CategoryTaxonomy, Product, etc.) is explicitly out of scope here — worth a dedicated follow-up issue, not silently bundled into this spec.
...
- **FR-014**: The system MUST expose Namespace watch through a dedicated, strongly-typed GraphQL subscription (e.g. `namespaceEvents`) whose payload is a concrete Namespace event type, not a generic/union type requiring a client to discriminate on a `kind` field — in addition to (not as a replacement for) the existing generic `watchResources(kind: "Namespace")` path.
- **FR-015**: The dedicated Namespace subscription MUST satisfy every other requirement in this contract (FR-001 through FR-013) identically to the generic path — the same race-free list-then-watch establishment, event shapes, resume/expiry semantics, ordering, and delivery guarantees — so a consumer gets no weaker a contract by using the typed entry point.
- **FR-016**: The documented contract's consumer-facing examples (FR-011) MUST be written against the dedicated `namespaceEvents` subscription as the primary, recommended form, with the generic `watchResources(kind: "Namespace")` form noted only as the pre-existing alternative.
...
- **Dedicated Namespace subscription (`namespaceEvents`)**: A strongly-typed GraphQL subscription whose payload is a concrete Namespace event type, serving the exact same underlying event stream as the generic `watchResources(kind: "Namespace")` path but without a stringly-typed `kind` parameter or a generic/union response type. The canonical, documented entry point for new Namespace watch consumers going forward; the generic path remains available but is no longer the recommended form.
...
- **SC-007**: A consumer can observe every Namespace watch event through a dedicated, strongly-typed `namespaceEvents`-style subscription without ever needing to pass a `kind` string parameter or discriminate a generic/union payload by type — verified by the documented schema and its examples.
...
- There is exactly one watch entry point for Namespace today (the generic, kind-parameterized `watchResources` mechanism). Per user design feedback, this spec reverses its original position and now requires a second, dedicated, strongly-typed entry point (`namespaceEvents`, FR-014–FR-016) for Namespace specifically, on the grounds that core resource kinds should be observable through typed schema rather than a stringly-typed `kind` discriminator. This is scoped as an additive change (the generic path is not removed or deprecated by this spec) and is limited to Namespace; giving the same treatment to other already-shipped core kinds is explicitly out of scope and should be tracked as its own follow-up.
