...
- The `BOOKMARK` event type (FR-017) already exists in the shared `WatchEventType` enum and already applies uniformly to every kind's watch stream; this spec does not introduce it, it corrects an omission in this spec's own initial documentation of the Namespace event shape.
