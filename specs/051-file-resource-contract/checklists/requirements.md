...
- Two genuine ambiguities were resolved with documented defaults instead of open questions, per the unattended-authoring instruction; one of them (below) was later revisited via a formal Clarification during design review:
  1. **(Revised)** The tension between GH#79's original, flat `status` lifecycle enum (`Uploaded`/`Processing`/`Ready`/`Failed`) and ADR-0008's `Condition`-list vocabulary was initially resolved by defining both fields, then revised once more (a strictly-computed rollup), and finally resolved by removing `status.phase` entirely — it's the pattern Kubernetes deprecated for `Pod.status.phase`, GH#79's own Acceptance Criteria never required it, and no other catalog resource kind has a `phase` field. See the Clarifications section (Session 2026-08-21) and FR-019.
