...
### User Story 3 - Read System-Populated FileStatus (Priority: P2)

An API consumer, operator, or another resource's read path inspects the `status` block of a `File` document to determine, at a glance, whether the file's binary payload has been uploaded, is being processed, is ready to reference, or has failed — without needing to interpret a detailed condition list.

**Why this priority**: Status fields are machine-written; their schema must be documented so consumers can rely on a stable, simple lifecycle signal (`Uploaded`/`Processing`/`Ready`/`Failed`) as well as the detailed structured conditions already used by other catalog resources, without inspecting raw file content or reverse-engineering condition semantics.

**Independent Test**: A `FileStatus` object with all documented fields (`phase`, `observedGeneration`, `lastAppliedRevision`, `conditions`, `resolved`) can be serialized and deserialized without data loss. Each condition type in the fixed set (`AdmissionAccepted`, `SourceResolved`, `ProcessingComplete`, `Ready`, `Terminating`) is representable in the model, and each `phase` value (`Uploaded`, `Processing`, `Ready`, `Failed`) is representable independently of the condition list.

**Acceptance Scenarios**:

1. **Given** a file status containing `phase`, `observedGeneration`, `lastAppliedRevision`, `conditions`, and a `resolved` block, **When** the system reads the status, **Then** all fields are accessible with correct types.
...
4. **Given** a newly admitted `File` document with no controller-driven processing yet performed, **When** the system reads the file, **Then** `status.phase` reads `Uploaded` rather than being absent, null, or erroring.
...
- **FR-012**: The `status` field MUST support the following sub-fields: `phase`, `observedGeneration`, `lastAppliedRevision`, `conditions`, and `resolved`. This field is stored in the datastore and merged with git content at read time; it is never persisted in the git repository.
- **FR-013**: `status.phase` MUST be one of the fixed lifecycle values `Uploaded`, `Processing`, `Ready`, or `Failed`, and MUST always be present and non-null once a `File` resource has been admitted — never absent or erroring, including immediately after admission before any further processing has occurred (in which case it reads `Uploaded`).
- **FR-014**: The `conditions` list MUST use only `True`, `False`, or `Unknown` as valid values for `conditions[].status`. The `conditions[].type` field MUST be one of the fixed enumeration for v1beta1: `AdmissionAccepted`, `SourceResolved`, `ProcessingComplete`, `Ready`, `Terminating`. No custom or additional condition types are permitted in this version.
- **FR-015**: The `status.resolved` block MUST support a `resolvedVariants` placeholder for controller-computed rendition output; this specification defines its presence in the schema only — populating it is Phase 2 controller work and out of scope here.
- **FR-016**: `metadata` MUST support author-supplied fields: `name`, `generateName`, `namespace`, `labels` (string map), and `annotations` (string map). The read-only fields `uid`, `resourceVersion`, `generation`, `creationTimestamp`, and `revision` are system-managed and stored in the datastore; they are merged into the resource view at read time and MUST NOT appear in author-pushed git files.
- **FR-017**: `metadata.ownerReferences` MUST support: `apiVersion`, `kind`, `name`, and `uid`, and MUST be system-managed (never author-writable).
- **FR-018**: The system MUST reject any author-pushed file that contains `status` or read-only `metadata` fields (e.g. `uid`, `resourceVersion`) via admission validation. These fields are system-managed and stored in the datastore only.
- **FR-019**: The system MUST document the full schema with a complete worked example matching the example in GH#79.
...
- **FileStatus**: System-written state stored in the datastore (not in git). Contains a simple lifecycle `phase` (`Uploaded`/`Processing`/`Ready`/`Failed`), reconciliation conditions, observed generation, last applied revision, and a `resolved` block reserved for controller-computed rendition output. Merged into the resource view at read time.
...
- `status.phase`, by contrast, is deliberately a stricter/more literal signal than the optimistic `Ready` condition: in this phase it reads `Uploaded` immediately after admission and does not advance to `Processing`, `Ready`, or `Failed` until Phase 2 controller work that verifies the source and runs processing actually exists (see Assumptions).
...
- **SC-004**: All `FileStatus` fields (`phase`, `observedGeneration`, `lastAppliedRevision`, `conditions`, `resolved`) are representable in the model without data loss, and every `File` resource returns a well-defined, non-null status the first time it is read after admission.
...
- **`status.phase` vs. the optimistic `Ready` condition**: ADR-0008 states that the `Ready` *condition* is set `True` immediately after `AdmissionAccepted=True` in Phase 1, even though source verification and processing have not actually occurred. This specification treats `status.phase` as a separate, stricter signal aimed at simple consumers who only want one field to check: `status.phase` reads `Uploaded` immediately after admission and does not advance to `Processing`, `Ready`, or `Failed` until the Phase 2 controller work that performs source verification and processing actually exists. This intentionally makes `status.phase` and the `Ready` condition disagree during Phase 1 (`phase=Uploaded` while `conditions[Ready]=True`) — a deliberate, documented asymmetry rather than a defect, so that a caller reading only `status.phase` is not told a file is "Ready" before its source has ever been verified.
