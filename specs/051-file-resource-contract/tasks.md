...
- [X] T041 [P] Add authenticated namespace-isolation and SecretRef boundary tests for File admission/status/watch paths in `gitstore-api/internal/auth`, `gitstore-api/internal/cataloggrpc`, and `gitstore-api/internal/graph`
  ...
  - `gitstore-api/internal/middleware/security/graphql_file_status_test.go`: exercises the *real* production authorization boundary — `GraphQLFieldAuthorizer`'s `updateResourceStatus` case derives and enforces the exact action string `file.status.write` (verified both Allow and Deny→FORBIDDEN), and explicitly pins down (rather than papers over) the current, real gap that `watchFiles`/`watchResources` subscriptions never reach this authorizer at all (`fc.Object != "Mutation"`), so there is no authenticated-boundary enforcement point for watch traffic to test today — namespace isolation for watch traffic instead comes from eventbus/resolver-level filtering (already covered by `TestWatchFiles_DeliversTypedPayloadAndFiltersNamespace`).
