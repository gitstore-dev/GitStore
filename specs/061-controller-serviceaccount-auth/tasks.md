...
- [ ] T031a [P] [US4] Rename `registerProductWatch` → `registerProduct` in `gitstore-controller-manager/cmd/controller/main.go` for consistency with its `registerNamespace`/`registerCategoryTaxonomy` siblings. Package-private with only 4 references (declaration at ~line 273, its doc comment at ~line 267, the call at ~line 89, and a comment reference in `tests/integration/product_category_count_test.go:314`) — a pure rename with no behavior change. Do this **in the same commit as T031**, which already rewrites all three signatures, so the rename does not become an independent diff reviewers must correlate. Note the name currently encodes a real distinction — it registers a watch/cache feeding another reconciler's queue rather than a reconciler of its own — so if that distinction is worth keeping, rename the *siblings* toward it instead of dropping it here; either way the three should agree
