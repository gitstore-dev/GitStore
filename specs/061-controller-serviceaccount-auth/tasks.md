...
- [ ] T030 [US4] Add `ServiceAccountNamespace`/`ServiceAccountName`/`ServiceAccountKeyID`/`ServiceAccountPrivateKeyFile` to `ControllerConfig` in `gitstore-controller-manager/internal/config/config.go`; make `ApiToken`'s required-check conditional on no signer being configured (mirroring T007's API-side pattern)
