...
| III. Clear Contracts & Versioning | PASS | All four mutations are net-new/additive; controller authentication intentionally removes the static API-token configuration path under the project's alpha breaking-change policy. |
...
- the controller-manager's `CredentialSource` interface makes dynamically renewed service-account credentials available to every GraphQL client call site; `StaticToken` remains an isolated test utility, not a controller configuration path;
...
│   │       └── migrations/007_service_account.cql   # 005=spec 047 (landed), 006=spec 050 (in flight)
│   ├── graph/resolver/
...
6. Add failing unit tests for `gitstore-controller-manager`'s `CredentialSource`/`ServiceAccountSource` (sign, exchange, cache, proactive renew, backoff-on-failure, and fail-closed startup without a signer). Implement `credential.go`, rewire `client.go`'s `do()`/`Subscribe()`, and consolidate `main.go`'s three call sites into one shared client/`CredentialSource` construction.
...
9. Add end-to-end integration coverage (`tests/integration/serviceaccount_auth_test.go`) and update `docs/implementation/020-pluggable_auth_architecture.md` (Phase 7 addendum), `docs/configuration.md`/equivalent (new service-account controller keys), and `docs/runbooks/controller-auth.md` (doc 021 §13's runbook, new). Run targeted tests, `make build`, `make test`, `make pr-ready`.
