...
curl -sS $API_URL -d '{"query":"mutation IssueToken($input: IssueServiceAccountTokenInput!) { issueServiceAccountToken(input: $input) { tokenRequest { status { token expirationTimestamp } } } }", "variables": {"input": {"metadata": {"namespace":"controllers","name":"gitstore-controller-manager"}, "spec": {}}}}' \
