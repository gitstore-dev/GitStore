...
- [X] T018 [P] [US2] Implement `HasCatalogResources` in `gitstore-api/internal/datastore/scylla/backend.go`: partition/index-scoped `LIMIT 1`-equivalent query per catalog table filtered by `repository_id`, short-circuiting on first match, avoiding `COUNT(*)` or unscoped scans across any table.
