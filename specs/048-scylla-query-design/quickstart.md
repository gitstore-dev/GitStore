...
| [#355](https://github.com/gitstore-dev/GitStore/issues/355) | Repository lookup and bounded Namespace/global listing | Existing pagination/envelope suites in `gitstore-api/tests/contract/datastore/{pagination,contract}_test.go`; memdb and Scylla backend tests; `tests/integration/repository_read_contract_test.go` | `specs/048-scylla-query-design/contracts/scylla-access-patterns.md`, `docs/resource-storage/README.md`, `docs/architecture/README.md` | `make test-scylla-hardening`; `make test-scylla-integration SCYLLA_TEST_ADDR=127.0.0.1:9042`; integration command shown above |
