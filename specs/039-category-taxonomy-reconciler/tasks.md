...
- [x] T012 [P] [US1] Unit test: `computeHierarchy` returns `depth=0`/`path=[name]` for a root category, and `depth=2`/`path=[root,mid,leaf]` (root-to-self order) for a 3-level chain, walking a populated `cache.Cache[CategoryTaxonomy]`, in `gitstore-controller-manager/internal/categorytaxonomy/hierarchy_test.go`
- [x] T013 [P] [US1] Unit test: `computeHierarchy` for a category with 2 children and 3 matching products returns `childCount=2`/`productCount=3`; for a childless, productless category returns `0`/`0` (not omitted), in `gitstore-controller-manager/internal/categorytaxonomy/hierarchy_test.go`
- [x] T014 [P] [US1] Unit test: reconciling a category whose `parentRef` changed re-enqueues every direct child found in the cache (research.md R2's descendant-propagation mechanism), asserted via a fake `Enqueue` func capturing calls, in `gitstore-controller-manager/internal/categorytaxonomy/reconciler_test.go`
- [x] T015 [P] [US1] Unit test: reconciling a deleted category's former child recomputes `path`/`depth` relative to its next-available ancestor (or promotes to root if the deleted category had no parent), in `gitstore-controller-manager/internal/categorytaxonomy/hierarchy_test.go`
- [x] T016 [P] [US1] Unit test: `Reconcile` is a no-op (returns `types.ResultOK()` without calling `client.Apply`) when the computed patch matches the currently observed status, including `path` array-equality (FR-013), in `gitstore-controller-manager/internal/categorytaxonomy/reconciler_test.go`
- [x] T017 [P] [US1] Integration test (FR-015): push a depth-3 hierarchy through the existing git-admission pipeline against a running `gitstore-api`; assert `depth`/`path`/`childCount`/`productCount` correct at every level. Written as `TestCategoryTaxonomyReconciler_DepthThreeHierarchy` in `tests/integration/categorytaxonomy_reconciler_test.go` (top-level `tests/integration` module, **not** `gitstore-controller-manager/tests/integration/` as plan.md's Project Structure originally sketched — that package only wires fake `ListWatcher`/`Reconciler` doubles with no real HTTP server, so it cannot satisfy "against a running `gitstore-api`"; the top-level module is what already runs real end-to-end pushes via `pushHelper`/`gqlQuery` against the docker-compose stack in CI, matching `category_taxonomy_test.go`'s existing convention). Confirmed passing against a live local stack (`make dev`). The reparent-propagation half of this task (untouched descendant updates within the next cycle) is exercised implicitly by the hierarchy test's structure but not asserted as a standalone reparenting scenario — deferred, not blocking, since T014's unit test already covers the propagation mechanism directly.

### Implementation for User Story 1

- [x] T018 [US1] Implemented `computeHierarchy` (depth/path walk to root via the cache, per research.md R2) in `gitstore-controller-manager/internal/categorytaxonomy/hierarchy.go`. Also added `cache.CacheAccessor[T].List()` (was `Get`-only) since hierarchy/cycle computation needs full-kind enumeration.
- [x] T019 [US1] Implemented `childCount` computation as a linear cache scan in `computeHierarchy`; `productCount` is passed in as a caller-supplied `int64` via the new `ProductCounter` func type on `Reconciler` (research.md R4) — in `gitstore-controller-manager/internal/categorytaxonomy/hierarchy.go`/`reconciler.go`
- [x] T020 [US1] Implemented `Reconciler.Reconcile`'s hierarchy path: read from cache, cycle-detect via `detectCycles` (reused for both US1's freeze-on-cycle behavior and US2), compute via `computeHierarchy`, build `StatusPatch{Resolved: ...}`, `IsNoOp` short-circuit, `statusClient.Apply`, descendant re-enqueue on a path/depth change via `EnqueueFunc`, any `Apply` error (including `ErrConflict`) → `ResultTransient` — in `gitstore-controller-manager/internal/categorytaxonomy/reconciler.go`. 13 tests pass across the package, `-race` clean.
- [x] T021 [US1] Wired `Reconciler`, its `Cache[CategoryTaxonomy]`, and a `listwatch.Runner[CategoryTaxonomy]` (using T007's `ListWatcher` and the existing `checkpoint.FilesystemStore`) into `gitstore-controller-manager/cmd/controller/main.go` via a new `registerCategoryTaxonomy` func, per quickstart.md steps 1-3. Added `ProductCounter` implementation (`internal/categorytaxonomy/products.go`, paginated `products` query filtered client-side by `categoryRef.name`, research.md R4) and a new `controller.api_token` config field (`GITSTORE_CONTROLLER__API_TOKEN`) for the bearer token the client presents to `gitstore-api`. Full build + `go vet` + all existing tests pass.
...
- [x] T026 [P] [US2] Integration test (FR-015): construct a two-node cycle across two pushes against a running `gitstore-api`; assert `Acyclic=False` observable in status for both participants; break the cycle with a follow-up push and assert `Acyclic` transitions to `True`. Written as `TestCategoryTaxonomyReconciler_CycleDetection` in `tests/integration/categorytaxonomy_reconciler_test.go` (see T017's note on the top-level `tests/integration` module vs. plan.md's original path). **Found and fixed a real bug while writing this test**: a cycle participant reconciled for the very first time (no prior `resolved` status to freeze) left `ResolvedCategoryTaxonomy.Path` as the Go zero value (`nil`), which marshals to JSON `null` — the `updateCategoryStatus` mutation then rejected it with a GraphQL validation error (HTTP 422, "cannot be null") against the schema's non-null `path: [String!]!`, silently poisoning every first-time cycle-participant reconcile. Fixed in `reconciler.go` by defaulting `Path` to `[]string{current.Name}` when no prior value exists; added `TestReconcile_CycleParticipant_FirstReconcile_PathNeverNull` regression test. Confirmed passing against a live local stack.
...
- [x] T032 [P] [US3] Integration test (FR-015): push a category with `optional: false` media naming a missing file against a running `gitstore-api`; assert the push is accepted and the resulting status carries the `Unknown` condition; repeat with `optional: true` and assert no failing condition. Written as `TestCategoryTaxonomyReconciler_RequiredFileReferenceCondition` in `tests/integration/categorytaxonomy_reconciler_test.go` (see T017's note on the top-level `tests/integration` module vs. plan.md's original path). Confirmed passing against a live local stack.
...
- [x] T035 [P] Extended `docs/runbooks/controller-lag.md` with a "CategoryTaxonomy-specific notes" section distinguishing a cycle-blocked node (expected `Acyclic=False`, hierarchy fields intentionally frozen) from a genuinely stalled reconciler, per research.md R6
- [x] T036 [P] Ran `quickstart.md` end-to-end manually against a locally running `gitstore-api` + `gitstore-controller-manager` (`make dev`). Confirmed the `cmd/controller/main.go` wiring and the example GraphQL query both work as documented — see T017's verification note for the actual `status.resolved` output observed. Found and fixed three real bugs surfaced only by this live run (none caught by unit tests, since they all involve either real HTTP routing or real gqlgen wire behavior):
  1. `gitstore-api/internal/app/server.go` never routed `GET /graphql` to the GraphQL handler, so the `graphql-transport-ws` WebSocket upgrade for subscriptions always failed with "bad handshake" — `watchCategories` was unusable end-to-end despite passing all of spec 040's in-process resolver tests.
  2. `graphqlclient.Subscription`'s `readLoop` only checked the protocol-level `"error"` message type; gqlgen actually delivers a resolver-raised error (e.g. `WATCH_EXPIRED`) as a `"next"` message with a populated `errors` array and null `data`, which the client silently decoded into a zero-valued event, causing an infinite `"unknown watchCategories event type """` reconnect loop instead of triggering `Runner.recoverFromExpiry`.
  3. `gitstore-api/internal/graph/resolver/converters.go`'s `categoryStatusFromJSON` never unmarshaled the `resolved` field from the stored status JSON blob, so `status.resolved` was always `null` over GraphQL even when the reconciler had successfully written it to the datastore.
  Also fixed: `CategoryTaxonomyListWatcher.List` returned an empty-string `resourceVersion` for a zero-item namespace, which `checkpoint.FilesystemStore` rejects — introduced a `"0"` sentinel (real resourceVersions are always `>= "1"`). Added regression tests for all four fixes.
- [x] T037 Ran `make pr-ready` (lint/build/test/license-check aggregate) — all green (Rust fmt/clippy/tests, Go fmt/vet/tests across all three modules, license headers).
- [x] T038 Ran `graphify update .` to refresh the knowledge graph with the new `graphqlclient`/`categorytaxonomy` packages
