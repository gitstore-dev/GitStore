...
- [X] T010 [US1] Write `TestIntegration_Restart_ResumesFromCheckpoint_NoLostOrDuplicateWork` in `gitstore-controller-manager/tests/integration/reconcile_retry_resume_test.go`: using `newRunner` (T005) and a shared `checkpoint.MemoryStore`, bootstrap a `Runner[widget]` + `manager.Manager`, reconcile some items to success, cancel the context mid-run with other items still pending/in-flight, then construct a second `Runner[widget]` + `manager.Manager` pair against the same `Store` and start it. **Deviation from original task text**: verified against the real `Runner.restoreCheckpoint` implementation that resume re-enqueues every cached item as a level-triggered resync (matching existing coverage in `tests/contract/listwatch_resume_test.go::TestRunner_Resume_RestoresCacheAndReplaysDurableWork`) rather than skipping already-completed items — the test instead asserts the three guarantees FR-003 actually requires: no `List` call on resume, the previously-pending item reaches completion, and no item is reconciled more than once within the resume cycle (depends on T003-T007)
