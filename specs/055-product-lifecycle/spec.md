...
- **FR-030**: `deleteProduct` MUST return the current Product envelope and a mandatory `ProductDeletionOutcome` that distinguishes `TERMINATION_STARTED` from `ALREADY_TERMINATING`. It MUST NOT introduce a legacy `deletedIdentifier` field.
