...
**Created**: 2026-08-20
**Status**: Draft (replayed against `main` 2026-09-09 — see Clarifications Session 2026-09-09)
...
### Session 2026-09-09 (replay against current `main`)

- Q: Has the coexistence premise changed now that spec 052's category-direction `ownerReferences` shipped? → A: No — it is now concrete rather than hypothetical. `resolvedCategoryOwnerReferences` in `gitstore-api/internal/cataloggrpc/server.go` (and the GraphQL create path in `graph/resolver/product.resolvers.go`) already write `CategoryTaxonomy`-pointing entries server-side on Create/Update admission, with author-supplied values ignored. This spec's Repository-pointing entry joins the same array per FR-004/FR-009 — each direction touches only its own `kind` entry. The shipped spec-052 mechanism is also the reference implementation for how this spec's write should look (server-computed, non-author-writable, no extra lookup).
- Q: Does `File` admission exist now (spec 051 landed)? → A: Yes — so FR-002 is directly actionable in this spec rather than forward-looking: the File admission path gets the same write.
- Q: Does the existence of the standalone `cmd/backfill-owner-references` command (category-direction, shipped with spec 052 via #373) change this spec's "no dedicated migration" decision? → A: No — the decision stands (admission-time write + self-healing Update suffice, per the issue's own acceptance criteria). But per spec 033's consolidation convention (`cmd/gitctl` replaced `cmd/hashpw`) and the 2026-09-09 review of that precedent, any future ops tooling this spec ever needs MUST land as a `gitctl` subcommand, not a new standalone `cmd/` — and the existing standalone backfill command is itself a fold-in candidate (tracked separately, not this spec's scope).
...
- What happens to a resource's Repository-pointing entry when spec 052's category/product-direction entries are added to or removed from the same record? They coexist in the same `metadata.ownerReferences` array without conflict — spec 052's mechanism is shipped (`resolvedCategoryOwnerReferences` in the catalog gRPC admission path and the GraphQL create path) and only ever inspects or modifies the entry matching its own `kind` (`CategoryTaxonomy`); this spec's write path likewise only ever touches the `Repository`-kind entry, leaving other entries in the array untouched.
...
- **FR-002**: The same requirement as FR-001 applies to the `File` resource kind, whose admission path exists as of spec 051 (already landed); this spec implements the write there too, it does not implement `File` admission itself.
...
- **`File` admission exists as of spec 051**: this spec's `File`-related requirement (FR-002) applies to that landed admission path directly; implementing `File` admission itself is out of scope and tracked separately.
