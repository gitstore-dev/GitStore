...
2. **Go datastore** (`gitstore-api/internal/datastore`): add failing tests for `NormalizeNamespaceContract`/`AdvanceNamespaceSpecVersion`/`AdvanceNamespaceSystemVersion` and for `UpdateNamespace`'s optimistic-concurrency conflict behavior (memdb and Scylla). Add the new columns, the memdb schema entry, and the query-first tables to the alpha baseline in `001_initial_schema.cql`. Implement until green.
...
#     apiVersion: "gitstore.dev/v1beta1"
#     kind: "Namespace"
#     metadata: { name: "beta-store" }
#     spec: { title: "Beta Store", tier: USER }
#   }) {
#     namespace { metadata { name generation resourceVersion } }
#   }
# }
