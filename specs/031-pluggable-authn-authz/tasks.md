...
- [X] T027 [P] Add `DecisionLogger` middleware in `gitstore-api/internal/auth/registry.go` or a new `gitstore-api/internal/auth/logging.go` — wraps `AuthZProvider.Authorize` and emits a structured zap log line with fields: `provider`, `subject`, `action`, `resource_kind`, `resource_name`, `outcome`, `reason`, `latency_ms` per Decision 2 in `020-pluggable_auth_architecture.md`
...
- [X] T030 [P] Update `docs/` — ensure `../../docs/implementation/020-pluggable_auth_architecture.md` reflects the final implemented state (anonymous provider placement, chain fallthrough behavior)
