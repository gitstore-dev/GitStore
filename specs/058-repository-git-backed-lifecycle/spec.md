...
6. **Given** a Repository returned from any create, update, lookup, or `node(id:)` path, **When** its identity is read, **Then** `metadata.uid` equals `id` exactly and is the canonical Relay-encoded Repository node ID; the raw datastore UID is never exposed through `metadata.uid`.
...
2. **Given** a repository manifest that includes a `status` block, **When** it is pushed, **Then** admission rejects the manifest and leaves the existing record unchanged; status is written only through the controller-authorized status path.
...
4. **Given** an authorized Repository controller with a current `resourceVersion`, **When** it calls `updateRepositoryStatus`, **Then** the system applies only its status patch and returns the typed Repository; a stale version returns a GraphQL error with `RESOURCE_VERSION_CONFLICT` and the current version in extensions for retry, while authorization, not-found, and infrastructure failures use their own GraphQL errors.

---

### User Story 6 - Repository controllers observe a durable, replica-safe lifecycle stream (Priority: P1)

A Repository controller must establish a complete current view without missing a concurrent create, admission update, status write, deletion marker, finalizer change, or final deletion. It must also resume safely after reconnecting to a different API replica or after a controller/API restart. The shipped Namespace watch architecture in spec 050 supplies the required model; Repository adopts that model for its own namespaced stream instead of relying on process-local events or polling.

**Independent Test**: A controller establishes a bootstrap watch, lists repositories in one namespace, drains queued events, then reconnects through a second API replica using its saved cursor while admission, reconciliation, and deletion transitions occur. It receives the complete ordered stream or an explicit expiry signal requiring a fresh bootstrap.

**Acceptance Scenarios**:

1. **Given** an authorized controller with no Repository cursor, **When** it establishes the documented bootstrap/list/drain sequence for a namespace, **Then** it obtains a complete snapshot and a durable cursor with no unobserved gap between the snapshot and subsequent events.
2. **Given** an authorized controller holding a retained Repository watch cursor, **When** it resumes through any healthy API replica, **Then** it receives only later `ADDED`, `MODIFIED`, `DELETED`, or `BOOKMARK` events in the Repository stream's order; duplicate delivery is tolerated by idempotent reconciliation.
3. **Given** a cursor whose continuity cannot be proven because of retention expiry, epoch mismatch, replay limit, overflow, or journal discontinuity, **When** it is used, **Then** the watch fails closed with `WATCH_EXPIRED` rather than ending normally or supplying a potentially incomplete stream.
4. **Given** an admitted Repository changes its spec, status, deletion marker, or finalizers, or is finally removed, **When** the authoritative datastore write commits, **Then** the Repository watch publishes the corresponding durable event; rejected, conflicting, and no-op operations publish none.
5. **Given** an unauthorized caller, **When** it attempts either the typed or generic Repository watch, **Then** authorization is denied before cursor parsing, replay, or any Repository identity is disclosed.
...
- **FR-009**: The system MUST reject any author-submitted `Repository` manifest containing `status` content. Status is never set by an authored manifest; it is written only through the controller-authorized status path.
...
- **FR-014**: The system MUST NOT hard-delete a repository's record until every condition required for safe removal has been satisfied — at minimum: `HasCatalogResources` remains false, the bare Git repository has been confirmed removed (or archived) from the git-service filesystem, and every finalizer has cleared. Background garbage collection MAY perform only that post-finalizer hard-delete; it MUST NOT cascade-delete catalog resources because their Repository is terminating.
...
- **FR-019**: The API MUST expose a controller-authorized `updateRepositoryStatus` mutation with the same partial-merge, spec-write exclusion, and `resourceVersion` precondition semantics as `updateCategoryStatus` and `updateResourceStatus`. A failed precondition MUST be a GraphQL error with stable `RESOURCE_VERSION_CONFLICT` and `currentResourceVersion` extensions, not an alternate successful payload; authorization, not-found, and unexpected operational failures MUST likewise remain GraphQL errors with extensions.
- **FR-020**: Every public `Repository.metadata.uid` value MUST be the exact same canonical Relay global ID as that Repository's `id`, encoded with the `Repository` node kind from the internal datastore UID. The API MUST NOT expose the raw datastore UID through `metadata.uid`; this invariant applies consistently to mutation results, direct lookups, connections, and `node(id:)` resolution.
- **FR-021**: The API MUST expose an additive, typed `watchRepositories(namespace: String!, selector: LabelSelectorInput, resourceVersion: String): RepositoryWatchEvent!` subscription. It is the canonical Repository watch entry point; the existing generic `watchResources(kind: "Repository")` path MUST remain compatible and use the identical underlying stream.
- **FR-022**: Repository watch events MUST carry `ADDED`, `MODIFIED`, `DELETED`, or `BOOKMARK`, an opaque Repository-journal cursor distinct from `Repository.metadata.resourceVersion`, the repository name and namespace for data events, and a full Repository postimage only for `ADDED`/`MODIFIED`.
- **FR-023**: Repository watch history, cursor allocation, replay, and live tailing MUST be durable and shared across API replicas, using the replica-safe journal, CDC materialization, lease/fencing, and append-before-progress model established by spec 050. The Repository stream MUST have a distinct cursor namespace and no cross-kind ordering guarantee.
- **FR-024**: The Repository controller MUST use the existing `ListWatcher`/`Runner`/checkpoint/cache architecture with the Repository bootstrap/list/drain protocol. Reconciliation MUST be idempotent because delivery is at least once, and `WATCH_EXPIRED` MUST cause a relist rather than continued operation from a stale cache.
- **FR-025**: Successful committed Repository creates, admitted spec updates, system status/finalizer/deletion-marker writes, and final deletion MUST produce durable Repository watch records derived from the authoritative repository row. Failed, rejected, conflicting, rolled-back, and no-op writes MUST produce no successful watch record.
- **FR-026**: Repository watch MUST authorize `repository.watch` before cursor parsing, journal reads, replay, or registration, for both typed and generic paths. Denied callers MUST not learn Repository names, namespaces, cursors, retention state, or payloads.
- **FR-027**: Repository watch replay, retention, buffers, polling/backoff, bookmarks, materializer lease, and readiness MUST be explicitly bounded. Any continuity failure MUST fail closed with stable `WATCH_EXPIRED` or `WATCH_UNAVAILABLE` errors; it MUST never silently skip a committed Repository transition.
- **FR-028**: Repository watch rollout MUST be migration- and server-first. During mixed journal semantics, Repository subscriptions are denied fleet-wide; enablement occurs only after every API replica, CDC/materializer, journal schema, and readiness checks have converged. Rollback restores the deny before reverting readers.
- **FR-029**: Repository admission MUST write exactly one system-owned Namespace owner reference for every newly admitted Repository: `{apiVersion: "gitstore.dev/v1beta1", kind: "Namespace", name: metadata.namespace, uid: <Namespace persistent UID>, blockOwnerDeletion: true}`. A Repository manifest containing owner references MUST be rejected. This closes the Namespace→Repository ownership gap for new rows rather than waiting for a backfill; any later backfill repairs legacy rows only. Namespace deletion continues to enforce its existing repository-existence fence.
...
- **Repository watch cursor**: Opaque, durable per-kind journal position used only to resume the Repository lifecycle stream. It is distinct from the per-row optimistic-concurrency `metadata.resourceVersion` and is never constructed or compared by callers.
- **Repository ownership**: Repository is scoped by `metadata.namespace` and has one system-owned Namespace owner reference with `blockOwnerDeletion: true`. The reference is persisted with the Namespace's internal UID, following the existing owner-reference storage convention; GraphQL must expose its canonical public identity consistently with the owner-reference contract.
...
For every returned Repository, `metadata.uid` is not a separate raw persistence identifier: it is exactly equal to the opaque Relay `id`. Both encode the Repository node kind and its internal datastore UID; clients must treat them as the same public identity token.
...
Author-submitted manifests containing a `status` block are rejected. The controller-authorized status path owns the values for `status`, `observedGeneration`, and `resourceVersion` after admission. `StorageProvisioned` deliberately uses ADR-0003's own condition name (not `SystemRepoReady`, which is Namespace-specific to spec 046) since a `Repository`'s own storage is the bare Git repository on disk, not another repository.
...
- **SC-008**: 100% of Repository results across mutation, lookup, connection, and `node(id:)` paths expose `metadata.uid` exactly equal to `id`, with zero raw datastore UIDs exposed through `metadata.uid`.
- **SC-009**: In a two-API-replica test, 100% of committed Repository lifecycle transitions are observed through either Repository watch endpoint in stream order, including a resume through the other replica; duplicate delivery is permitted, but zero acknowledged transitions are missing.
- **SC-010**: 100% of expired, incompatible, overflowed, or discontinuous Repository cursors terminate with `WATCH_EXPIRED`, and zero such cases end as an apparently successful stream.
- **SC-011**: The two-replica Repository watch gate demonstrates visibility p95 ≤1 second, p99 ≤3 seconds, 10,000-event replay p95 ≤5 seconds, recovery after API replacement ≤30 seconds, and zero missing acknowledged Repository transitions.
...
- The future "Repository Validation and Admission Matrix" still owns the structural-vs-policy rule catalogue beyond this lifecycle contract. Repository watch is no longer deferred: this spec adopts the shipped spec-050 durable watch architecture as part of the controller lifecycle it introduces.
