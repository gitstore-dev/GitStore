...
**Decision**: Both mutations keep their existing GraphQL input/output shapes (`RenameRepositoryInput`/`RenameRepositoryPayload`, `TransferRepositoryInput`/`TransferRepositoryPayload`) but change in two coupled ways, shipped together: (1) their resolvers are rewritten to unconditionally return an `Unimplemented`-classed error and perform no datastore or Git mutation whatsoever, deleting the current working implementation in `Service.RenameRepository`/`Service.TransferRepository`; and (2) their field definitions in `shared/schemas/repository.graphqls` gain a `@deprecated(reason: "...")` directive citing ADR-0003's Phase 2 deferral.

**Rationale**: ADR-0003's own §"GraphQL mutation delegation" table states the runtime outcome directly: "**Not supported in Phase 1.** Returns `Unimplemented`." The current shipped code contradicts this by performing real, unreviewable datastore-only renames/transfers — behavior that would silently diverge a repository's record from its Git manifest once create/update/delete become git-backed, breaking the "Git is canonical" invariant this spec establishes for every other mutation. Keeping the schema shape (rather than removing the fields) preserves Principle III (no breaking schema removal) while still delivering the correctness-critical behavior change. The `@deprecated` directive is added for the same reason spec 045 added it to Repository's own legacy flat output fields, and the Namespace/CategoryTaxonomy contract specs added it before that: this project's established convention is to proactively signal phase-out through schema introspection, not leave callers to discover it only by hitting a runtime error. Deprecation without the runtime error would be dishonest (the mutations would still silently "work" via datastore writes); the runtime error without deprecation would leave schema tooling with no advance signal — both halves are required together.

This decision explicitly supersedes spec 045's Acceptance Scenario #4 ("the current `createRepository`, `renameRepository`, `transferRepository`, and `deleteRepository` operations ... continue to succeed and fail under exactly the same conditions as before") and SC-003 (100% conformance to that invariant), for `renameRepository`/`transferRepository` only. That invariant was correct for spec 045, a read-schema-only change; it cannot remain correct once this spec makes create/update/delete git-backed, and ADR-0003 already specifies that these two mutations in particular should stop working in Phase 1. This is recorded as an intentional, named regression against spec 045's own stated invariant, not an oversight — `createRepository`/`deleteRepository` are not part of this supersession; their success/failure conditions are governed by this spec's own admission/finalizer rules instead.
...
- *Return `Unimplemented` at runtime without also adding `@deprecated`*. Rejected — this delivers the enforcement half but not the advance-notice half, leaving schema-level tooling (codegen, linting, admin-console introspection) with no signal that these two mutations are being phased out, inconsistent with how spec 045 already treated Repository's own legacy fields.
- *Add `@deprecated` now but leave the current working rename/transfer implementation in place until a later spec*. Rejected — deprecation without enforcement would let callers keep depending on unreviewable datastore-only renames/transfers indefinitely, and would not close ADR-0003's actual gap.
