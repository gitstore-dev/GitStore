...
│   └── repository-lifecycle.graphqls # createRepository/updateRepository envelope; renameRepository/transferRepository Unimplemented + @deprecated notice
├── checklists/
│   └── requirements.md
...
4. Add the `updateRepository` mutation, `UpdateRepositoryInput`/`UpdateRepositoryPayload`, and envelope-shaped `CreateRepositoryInput` to `shared/schemas/repository.graphqls`; add `@deprecated(reason: "Not supported; see docs/ADRs/0003-repository-lifecycle.md Phase 2. Delete and recreate instead.")` (or equivalent wording) to the existing `renameRepository`/`transferRepository` field definitions in the same file; regenerate gqlgen code (never hand-edited); add a failing schema-introspection test asserting both fields are deprecated with a non-empty reason before making the change.
