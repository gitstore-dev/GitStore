...
**Decision**: HTTP `POST /controller/v1/poison/{kind}/{namespace}/{name}/requeue` — `net/http` handler, no new libraries. Atomically removes from quarantine store, resets retry budget, re-enqueues through the rate limiter.
