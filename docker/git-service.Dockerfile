...
# Copy buf-generated proto stubs (generated by buf.gen.rust.yaml; build.rs is a no-op)
COPY gitstore-git-service/gen ./gen
...
# Refresh mtimes and remove stale build-script output dirs.
RUN --mount=type=cache,id=cargo-registry-$TARGETARCH,target=/usr/local/cargo/registry \
    --mount=type=cache,id=cargo-git-$TARGETARCH,target=/usr/local/cargo/git \
    --mount=type=cache,id=cargo-target-$TARGETARCH,target=/build/target \
